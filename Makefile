...
install-bluerecording:  ## Install with bluerecording dependencies (for extracellular recording)
	...
	HDF5_MPI=ON uv pip install --no-cache --no-binary h5py h5py --no-build-isolation
	@echo "=== Building neurodamus-models ==="
	@if [ ! -d "$(NEURODAMUS_MODELS_DIR)" ]; then \
		git clone --depth=1 $(NEURODAMUS_MODELS_REPO); \
	fi
	$(eval NEURODAMUS_PYTHON := $(shell uv run python -c "import neurodamus; from pathlib import Path; print(Path(neurodamus.__file__).parent / 'data')"))
	cmake -B $(NEURODAMUS_MODELS_DIR)/build -S $(NEURODAMUS_MODELS_DIR)/ \
