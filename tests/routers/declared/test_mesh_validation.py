...
from app.endpoints.mesh_validation import router as mesh_router
...
def test_validate_mesh_file_empty(client, empty_mesh_upload, tmp_path):
    ...
    with (
        ...
        patch("app.endpoints.mesh_validation._cleanup_temp_file") as mock_cleanup,
    ):
...
def test_validate_mesh_file_too_large(client, valid_mesh_upload):
    ...
    with patch("app.endpoints.mesh_validation._save_upload_to_tempfile", 
               side_effect=Exception("File size exceeds the limit")): # Generic catch or specific
        # Re-importing inside to access the custom exception if needed, 
        # but here we mock the behavior of the catch block
        from app.endpoints.mesh_validation import FileTooLargeError
        
        with patch("app.endpoints.mesh_validation._save_upload_to_tempfile", side_effect=FileTooLargeError("Too big")):
            response = client.post(ROUTE, files=valid_mesh_upload)
...
def test_validate_mesh_file_reader_fails(client, valid_mesh_upload, tmp_path):
    ...
    with (
        patch("app.endpoints.mesh_validation._save_upload_to_tempfile", return_value=str(saved_path)),
        patch("app.endpoints.mesh_validation.validate_mesh_reader", side_effect=RuntimeError("Trimesh explosion")),
    ):
        response = client.post(ROUTE, files=valid_mesh_upload)
...
def test_validate_mesh_file_os_error(client, valid_mesh_upload):
    with patch("app.endpoints.mesh_validation._save_upload_to_tempfile", side_effect=OSError("Disk full")):
        response = client.post(ROUTE, files=valid_mesh_upload)
