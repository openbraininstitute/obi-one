...
def _make_user_context(*, virtual_lab_id=None, token_credentials="fake-token"):  # noqa: S107
...
def test_no_placeholder_in_url(monkeypatch):
...
def test_no_virtual_lab_id_raises(monkeypatch):
...
def test_compute_cell_a(monkeypatch, httpx_mock):
...
def test_compute_cell_b(monkeypatch, httpx_mock):
...
def test_unknown_compute_cell_raises(monkeypatch, httpx_mock):
...
def test_api_failure_raises(monkeypatch, httpx_mock):
