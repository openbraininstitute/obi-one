...
def mock_io_for_test(monkeypatch):[cite: 2]
    ...
    monkeypatch.setattr("app.endpoints.morphology_metrics_calculation.pathlib.Path", mock_path_constructor_final)
    monkeypatch.setattr("app.endpoints.morphology_metrics_calculation.Path", mock_path_constructor_final)
...
def test_morphology_registration_success(client, monkeypatch, mock_entity_payload):[cite: 2]
    ...
    monkeypatch.setattr("app.endpoints.morphology_metrics_calculation._run_morphology_analysis", lambda _: [])

    response = client.post(ROUTE, data={"metadata": mock_entity_payload}, files={"file": ("test.swc", b"content")})
    
    assert response.status_code == 200
    assert response.json()["entity_id"] == mock_id
    client.app.dependency_overrides.clear()

@pytest.mark.parametrize("filename, content, metadata, expected_code", [[cite: 1]
...
def test_internal_errors(client, monkeypatch, mock_entity_payload):[cite: 1]
    def mock_fail(*args, **kwargs): raise Exception("Neurom error")
    monkeypatch.setattr("app.endpoints.morphology_metrics_calculation._run_morphology_analysis", mock_fail)
...
def test_meshing_failure_is_graceful(client, monkeypatch, mock_entity_payload):[cite: 1]
    ...
    monkeypatch.setattr("app.endpoints.morphology_metrics_calculation._mesh_and_register", 
