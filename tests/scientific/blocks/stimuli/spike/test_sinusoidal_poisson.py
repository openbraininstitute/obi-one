...
class TestSinusoidalPoissonSpikeStimulus:
    def test_spikes_within_time_window(self, tmp_path):
        ...
        timestamps = obi.RegularTimestamps(
            start_time=100.0, number_of_repetitions=1, interval=1000.0
        )
        duration = 500.0

        vc = make_validated_stimulus(
            source, target, SinusoidalPoissonSpikeStimulus, timestamps,
            duration=duration, minimum_rate=1.0, maximum_rate=20.0,
            modulation_frequency_hz=5.0, random_seed=42,
        )
    ...
    def test_reproducible_with_same_seed(self, tmp_path):
        kwargs = dict(
            duration=300.0, minimum_rate=1.0, maximum_rate=10.0,
            modulation_frequency_hz=5.0, random_seed=7,
        )
        ...
        vc1 = make_validated_stimulus(
            source1, target1, SinusoidalPoissonSpikeStimulus, ts1, **kwargs
        )
        stim1, _, pop1 = generate_spikes_for_stim(vc1, tmp_path / "r1")
        ...
        vc2 = make_validated_stimulus(
            source2, target2, SinusoidalPoissonSpikeStimulus, ts2, **kwargs
        )
        stim2, _, pop2 = generate_spikes_for_stim(vc2, tmp_path / "r2")
...
class TestSinusoidalPoissonValidation:
    def test_max_rate_less_than_min_rate_raises(self):
    ...
    def test_overlapping_timestamps_raises(self, tmp_path):
        ...
        vc = make_validated_stimulus(
            source, target, SinusoidalPoissonSpikeStimulus, timestamps,
            duration=200.0, minimum_rate=1.0, maximum_rate=10.0,
