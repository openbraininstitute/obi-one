...
from .conftest import STIM_CIRCUIT, generate_spikes_for_stim, make_validated_stimulus


def test_resolve_neuron_set_ref_returns_default_when_none():
    result = resolve_neuron_set_ref_to_node_set(None, "MyDefault")
    assert result == "MyDefault"


def test_generate_config_uses_default_node_set_when_target_is_none():
...
def test_generate_spikes_with_none_source_raises(tmp_path):
    sc = obi.CircuitSimulationScanConfig.empty_config()
    stim = FullySynchronousSpikeStimulus()
    sc.add(stim, name="stim")
...
def test_synchronous_generates_spikes_at_time_zero(tmp_path):
...
def test_poisson_with_none_timestamps_uses_default(tmp_path):
    source = obi.IDNeuronSet(neuron_ids=obi.NamedTuple(name="src", elements=range(3)))
    target = obi.IDNeuronSet(neuron_ids=obi.NamedTuple(name="tgt", elements=range(10)))
