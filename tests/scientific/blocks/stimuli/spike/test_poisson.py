...
class TestPoissonSpikeStimulus:
    def test_spikes_within_time_window(self, tmp_path):
        ...
        vc = make_validated_stimulus(
            source, target, PoissonSpikeStimulus, timestamps,
            duration=duration, frequency=50.0, random_seed=42, timestamp_offset=offset,
        )
    ...
    def test_reproducible_with_same_seed(self, tmp_path):
        kwargs = dict(duration=500.0, frequency=10.0, random_seed=123)
        ...
        vc1 = make_validated_stimulus(source1, target1, PoissonSpikeStimulus, ts1, **kwargs)
        stim1, _, pop1 = generate_spikes_for_stim(vc1, tmp_path / "run1")
        ...
        vc2 = make_validated_stimulus(source2, target2, PoissonSpikeStimulus, ts2, **kwargs)
        stim2, _, pop2 = generate_spikes_for_stim(vc2, tmp_path / "run2")
    ...
    def test_different_seed_produces_different_spikes(self, tmp_path):
        source1 = obi.IDNeuronSet(neuron_ids=obi.NamedTuple(name="s1", elements=range(3)))
        target1 = obi.IDNeuronSet(neuron_ids=obi.NamedTuple(name="t1", elements=range(10)))
        ts1 = obi.RegularTimestamps(start_time=0.0, number_of_repetitions=1, interval=1000.0)
        vc1 = make_validated_stimulus(
            source1, target1, PoissonSpikeStimulus, ts1,
            duration=500.0, frequency=10.0, random_seed=1,
        )
        ...
        vc2 = make_validated_stimulus(
            source2, target2, PoissonSpikeStimulus, ts2,
            duration=500.0, frequency=10.0, random_seed=999,
        )
...
class TestPoissonOverlapValidation:
    def test_overlapping_timestamps_raises(self, tmp_path):
