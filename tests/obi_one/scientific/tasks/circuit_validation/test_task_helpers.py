...
class TestGetPopulationSizes:
    def test_reads_sizes(self):
        ...
        mock_pop_b.size = 10
        mock_circuit.nodes.__getitem__ = lambda _self, k: (
            mock_pop_a if k == "pop_a" else mock_pop_b
        )
...
class TestValidateMorphologyPaths:
    ...
    def test_missing_morphology_file(self, tmp_path):
    ...
    def test_get_filepath_raises(self, tmp_path):
...
class TestValidateEmodelPaths:
    def _mock_pop(self, *, hoc_dir: str, templates: list[str], pop_type: str = "biophysical"):
        mock_pop = MagicMock()
        mock_pop.type = pop_type
        mock_pop.config = {"biophysical_neuron_models_dir": hoc_dir} if hoc_dir else {}
        mock_pop.property_names = {"model_template"} if templates else set()
        series = MagicMock()
        series.unique.return_value.tolist.return_value = templates
...
class TestValidateHocLoading:
    @patch("obi_one.scientific.tasks.circuit_validation.task._collect_hoc_files")
    @patch("obi_one.scientific.tasks.circuit_validation.task._find_morphology_for_template")
    def test_no_hoc_files_returns_empty(self, mock_find_morph, mock_collect, tmp_path):  # noqa: ARG002
        from obi_one.scientific.tasks.circuit_validation.task import (  # noqa: PLC0415
            _validate_hoc_loading,
        )
        ...
        result = _validate_hoc_loading(MagicMock(), tmp_path, load_mods=False)
        assert result == []

    @patch("obi_one.scientific.tasks.circuit_validation.task._collect_hoc_files")
    @patch("obi_one.scientific.tasks.circuit_validation.task._find_morphology_for_template")
    def test_hoc_no_morphology_skipped(self, mock_find_morph, mock_collect, tmp_path):
        from obi_one.scientific.tasks.circuit_validation.task import (  # noqa: PLC0415
            _validate_hoc_loading,
        )
        ...
        config_path = tmp_path / "circuit_config.json"
        config_path.write_text("{}")
        mock_collect.return_value = [hoc_file]
    ...
    def test_hoc_instantiation_failure(self, mock_init, mock_find_morph, mock_collect, tmp_path):
        from obi_one.scientific.tasks.circuit_validation.task import (  # noqa: PLC0415
            _validate_hoc_loading,
        )
    ...
    @patch("obi_one.scientific.tasks.circuit_validation.task._find_morphology_for_template")
    @patch("obi_one.scientific.validations.emodels.bluecellulab_initializable")
    def test_hoc_success(self, mock_init, mock_find_morph, mock_collect, tmp_path):
        from obi_one.scientific.tasks.circuit_validation.task import (  # noqa: PLC0415
            _validate_hoc_loading,
        )
