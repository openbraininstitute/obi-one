...
def test_build_synaptome_task_registers_circuit_and_updates_activity(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(BuildSynaptomeTask, "_get_execution_activity", get_activity)
    monkeypatch.setattr(BuildSynaptomeTask, "_update_execution_activity", update_activity)

    db_client = Mock()
    circuit_id = BuildSynaptomeTask(config=config).execute(
        db_client=db_client,
