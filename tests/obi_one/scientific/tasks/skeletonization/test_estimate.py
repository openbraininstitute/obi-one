...
@pytest.fixture
def mock_em_cell_mesh(mesh_id, mock_asset):
...
def mock_pylmesh():
    """Mock pylmesh module."""
    mock_mesh = Mock()
    mock_mesh.surface_area.return_value = 12.566  # Approximate surface area of unit sphere (4*pi)

    mock_module = Mock()
    mock_module.load_mesh.return_value = mock_mesh

    return mock_module


def test_compute_mesh_surface_area(mock_db_client, mesh_id, mock_pylmesh):
    """Test that _compute_mesh_surface_area correctly computes surface area."""
    with patch.dict("sys.modules", {"pylmesh": mock_pylmesh}):
        # Re-import to use the mocked module
        from obi_one.scientific.tasks.skeletonization.estimate import _compute_mesh_surface_area
        ...
        mock_pylmesh.load_mesh.assert_called_once()
...
def test_estimate_skeletonization_count_single_mesh(
    mock_db_client, config_id, mesh_id, tmp_path, mock_pylmesh
):
...
def test_estimate_skeletonization_count_multiple_meshes(
    mock_db_client, config_id, tmp_path, mock_pylmesh
):
    ...
    with patch.dict("sys.modules", {"pylmesh": mock_pylmesh}):
        # Create a mock config with multiple meshes
        config = SkeletonizationSingleConfig(
            info=Info(campaign_name="test", campaign_description="test"),
            initialize=SkeletonizationSingleConfig.Initialize(
                cell_mesh=[
        ...
        # With two meshes, the count should be double (2 * 12.566)
        assert count == 25.132
