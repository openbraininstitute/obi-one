...
def _make_poisson_stimulus(*, frequency: float, random_seed: int) -> obi.PoissonSpikeStimulus:
    ...
    _set_block_name(stimulus, "poisson_stimulus")
    return stimulus


def _patch_distribution_ref(
    monkeypatch: pytest.MonkeyPatch, stimulus: object, distribution: object
) -> None:
...
class TestDistributionSpikeStimulus:
    def test_generated_spike_file_has_correct_structure(self, tmp_path, monkeypatch):
        ...
        stimulus = _make_distribution_stimulus(duration=25.0, resample_each_repetition=False)
...
class TestPoissonSpikeStimulus:
    def test_uses_shared_distribution_path(self, monkeypatch):
        ...
        timestamps = obi.SingleTimestamp(start_time=0.0)
        stimulus = _make_poisson_stimulus(frequency=20.0, random_seed=42)
    ...
    def test_random_seed_forwarded_to_distribution(self, monkeypatch):
        neuron_set = obi.IDNeuronSet(neuron_ids=obi.NamedTuple(name="test", elements=[0]))
        timestamps = obi.SingleTimestamp(start_time=0.0)
        stimulus = _make_poisson_stimulus(frequency=10.0, random_seed=123)
