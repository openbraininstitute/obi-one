...
- ScanConfigs can optionally have the following specifed:
    - `properties_endpoint`: url relative to the base url of the api, such as "/circuit-usability/{circuit_id}". For providing data to a ui_element, see for example: [entity_property_dropdown](components/entity_property_dropdown/entity_property_dropdown.md). Within the returned dictionary a `usability` dictionary should also be defined with booleans which can be used for optionally displaying elements.
