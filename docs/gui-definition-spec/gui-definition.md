...
Scan configs can specify multiple UI elements. For example, there are UI elements for specifying a user-defined string. Each UI element corresponds to a variable of the ScanConfig, such as:
...
`ui_element` These are two major categories of UI elements. 

1. `Root UI elements` appear in the left hand column of the configuration interface. There are currently two supported types:
...
Scan configs can additionally have the following properties:
