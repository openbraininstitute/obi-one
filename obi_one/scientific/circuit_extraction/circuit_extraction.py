...
class CircuitExtraction(CircuitExtractions, SingleCoordinateMixin):
    ...
    def run(self, db_client: entitysdk.client.Client = None) -> str:
            ...
            if pop.config["type"] == "biophysical":
                ...
                if "morphology" in pop.property_names:
                    ...
                    for _morph_ext in ["swc", "asc", "h5"]:
                        try:
                            ...
                            if not Path(morph_folder).exists():
                                msg = f"ERROR: {_morph_ext} morphology folder/container does not exist!"
                                raise ValueError(msg)

                            if os.path.isdir(morph_folder):
                                if len(self._filter_ext(os.listdir(morph_folder), _morph_ext)) == 0:
                                    msg = f"ERROR: {_morph_ext} morphology folder does not contain morphologies!"
                                    raise ValueError(msg)
                    ...
                    for _morph_ext in src_morph_dirs:
                        if _morph_ext == "h5" and Path(src_morph_dirs[_morph_ext]).is_file():
                            ...
                            for morphology_name in tqdm.tqdm(
                                ...
                                src_file = (
                                    Path(src_morph_dirs[_morph_ext])
                                    / f"{morphology_name}.{_morph_ext}"
                                )
                                dest_file = (
                                    Path(dest_morph_dirs[_morph_ext])
                                    / f"{morphology_name}.{_morph_ext}"
                                )
        ...
        mod_folder = "mod"
        source_dir = Path(os.path.split(self.initialize.circuit.path)[0]) / mod_folder
        if Path(source_dir).exists():
            L.info("Copying mod files")
            dest_dir = Path(self.coordinate_output_root) / mod_folder
