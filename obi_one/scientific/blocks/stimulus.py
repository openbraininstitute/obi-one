...
class TargetedStimulus(Stimulus, ABC):
    """Base for stimuli that target either a node_set (cells) or generated locations."""
    ...
    neuron_set: (
        Annotated[
            ...
            Field(
                title="Neuron Set",
                description="Cells receiving the stimulus.",
    ...
    # expert: explicit locations precomputed
    compartment_set: (
        Annotated[
            CompartmentSetReference,
            Field(
    ...
    def config(
        ...
        self._default_timestamps = default_timestamps or SingleTimestamp(start_time=0.0)

        if self.neuron_set is not None:
    ...
    def _target_entry(self) -> dict[str, Any]:
        ...
        # If explicit compartment_set is provided, use it.
...
class RelativeConstantCurrentClampStimulus(TargetedStimulus):
...
class LinearCurrentClampStimulus(TargetedStimulus):
...
class RelativeLinearCurrentClampStimulus(TargetedStimulus):
...
class NormallyDistributedCurrentClampStimulus(TargetedStimulus):
...
class RelativeNormallyDistributedCurrentClampStimulus(TargetedStimulus):
...
class MultiPulseCurrentClampStimulus(TargetedStimulus):
...
class SinusoidalCurrentClampStimulus(TargetedStimulus):
...
class SubthresholdCurrentClampStimulus(TargetedStimulus):
...
class HyperpolarizingCurrentClampStimulus(TargetedStimulus):
