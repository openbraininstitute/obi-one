...
- IMPORTANT: All neuron sets in combined_with must have the same node_population as the base_neuron_set (to avoid multi-population neuron sets!) 
- IMPORTANT: When resolving a CombinedNeuronSet, recursive infinite loops may occur and should be detected/avoided! 
- Replaces: node_sets functionality of the old PropertyNeuronSet, i.e., one can now create a combined neurons set based on a PropertyNeuronSet and PredefinedNeuronSet(s) to have the same behavior as before 
"""