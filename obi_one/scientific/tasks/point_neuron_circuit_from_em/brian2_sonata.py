...
are borrowed from the Drosophila brain model as placeholders.
"""
...
def _node_dataframe(
    pt_root_ids: list[int], model_type: str, model_template: str | None = None
) -> pandas.DataFrame:
    data = {"model_type": model_type}
    if model_template is not None:
        data["model_template"] = model_template
    data["pt_root_id"] = numpy.asarray(pt_root_ids, dtype=numpy.int64)
    return pandas.DataFrame(index=numpy.arange(len(pt_root_ids)), data=data)


def _write_nodes(
    output_dir: Path, point_pt_root_ids: list[int], virtual_pt_root_ids: list[int]
) -> None:
    nodes_path = output_dir / _NODES_FILE
    point = CellCollection.from_dataframe(
        _node_dataframe(point_pt_root_ids, "brian2_point", f"json:{NEURON_MODEL}"), index_offset=0
    )
    ...
    if virtual_pt_root_ids:
        virtual = CellCollection.from_dataframe(
            _node_dataframe(virtual_pt_root_ids, "virtual"), index_offset=0
        )
...
def _write_edge_population(
    h5: h5py.File, name: str, edges: EdgeSet, source_population: str, target_population: str
) -> None:
    ...
    group0.create_dataset("w", data=edges.synapse_count.astype(numpy.float64) * W_SYN_MV)  # mV
...
def _write_edges(output_dir: Path, connectivity: ResolvedConnectivity) -> list[str]:
    ...
    with h5py.File(edges_path, "w") as h5:
        if len(connectivity.internal_edges.source_node_id):
            _write_edge_population(
                h5, internal_name, connectivity.internal_edges, POINT_POPULATION, POINT_POPULATION
            )
            to_index.append((internal_name, n_point, n_point))
        if len(connectivity.external_edges.source_node_id):
            _write_edge_population(
                h5, external_name, connectivity.external_edges, VIRTUAL_POPULATION, POINT_POPULATION
            )
...
def _write_node_sets(output_dir: Path) -> None:
    ...
    path.write_text(json.dumps({"All": {"population": POINT_POPULATION}}, indent=2))
...
def write_brian2_sonata_circuit(output_dir: Path, connectivity: ResolvedConnectivity) -> Path:
    ...
    _write_nodes(output_dir, connectivity.point_pt_root_ids, connectivity.virtual_pt_root_ids)
    edge_population_names = _write_edges(output_dir, connectivity)
