...
def run_circuit_validation(  # ruff: ignore[too-many-locals, too-many-statements]
    *,
    db_client: Client,
    circuit_id: UUID,
...
def _compute_population_dynamics(  # ruff: ignore[too-many-locals]
    pop: NodePopulation,
) -> tuple[dict[int, float], dict[int, float]]:
    """Compute holding/threshold per unique (template, morphology) pair, then broadcast."""
    from bluecellulab.circuit.circuit_access.definition import (  # ruff: ignore[import-outside-top-level]
        EmodelProperties,
    )
