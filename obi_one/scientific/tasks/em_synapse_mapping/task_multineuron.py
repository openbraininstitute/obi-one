...
class EMMultiNeuronSynapseMappingTask(Task):
    """Multi-neuron synapse mapping task.
    ...
    - Internal edges (biophysical -> biophysical) for connections within the set
    ...
    def execute(  # NOQA: PLR0914, PLR0915, C901, PLR0912
        ...
        *,
        ...
        execution_activity = EMMultiNeuronSynapseMappingTask._get_execution_activity(
        ...
        L.info("Merging spiny morphologies into combined multi-neuron file...")
        merge_spiny_morphologies(
        ...
        for bio_node_id, rn in enumerate(resolved_neurons):
            ...
            plot_mapping_stats(mapped_synapses_df, mesh_res).savefig(
                out_root / f"mapping_stats_neuron_{bio_node_id}.png"
            )
            plt.close("all")
        ...
        bio_pt_root_mapping = pandas.DataFrame(
            {"index": range(len(resolved_neurons))},
                    ...
                    if col not in coll_bio.properties:
                        if vals.dtype.kind == "f":
                            coll_bio.properties[col] = numpy.full(
                                len(resolved_neurons), numpy.nan, dtype=vals.dtype
                            )
                        elif vals.dtype.kind in {"i", "u"}:
                            coll_bio.properties[col] = numpy.full(
                                len(resolved_neurons), -1, dtype=vals.dtype
                            )
                        else:
                            coll_bio.properties[col] = numpy.full(
                                len(resolved_neurons), "", dtype=vals.dtype
                            )
        ...
        file_paths = {
            ...
            fn_merged_h5: str(out_root / fn_merged_h5),
        }

        for bio_idx in range(len(resolved_neurons)):
            stats_name = f"mapping_stats_neuron_{bio_idx}.png"
        ...
        registered_circuit_id = register_output_multiple(
            db_client=db_client,
