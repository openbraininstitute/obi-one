...
def estimate_circuit_simplification_count(*, db_client: Client, config_id: UUID) -> int:
    ...
    config_dict = db_sdk.get_task_config_asset_content(client=db_client, config=task_config)
    ...
    if isinstance(parent_circuit, CircuitFromID):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            neuron_ids = target_neuron_set_ref.block.get_neuron_ids(circuit=staged_circuit)
    else:
        neuron_ids = target_neuron_set_ref.block.get_neuron_ids(
