...
class IonChannelModelSimulationExecutionConfig(Task, SingleConfigMixin):
    _single_entity: models.Simulation

    @property
    def single_entity(self) -> models.Simulation:
        return self._single_entity

    @single_entity.setter
    def single_entity(self, entity: models.Simulation) -> None:
        self._single_entity = entity


def get_task_config(
    client: entitysdk.Client, simulation: models.Simulation
) -> IonChannelModelSimulationSingleConfig:
    task_config_asset = client.select_assets(
        entity=simulation, selection={"label": AssetLabel.simulation_generation_config}
    ).one()

    content = client.download_content(
        entity_id=simulation.id, entity_type=models.Simulation, asset_id=task_config_asset.id
    )

    return IonChannelModelSimulationSingleConfig.model_validate_json(content)


class IonChannelModelSimulationTask(Task):
    config: IonChannelModelSimulationExecutionConfig
    ...
    def execute(
        ...
        *,
        ...
        task_config = get_task_config(client=db_client, simulation=simulation_entity)
        ...
        staged_circuit = stage_ion_channel_models_as_circuit(
            client=db_client,
            ion_channel_models=task_config.ion_channel_models,
