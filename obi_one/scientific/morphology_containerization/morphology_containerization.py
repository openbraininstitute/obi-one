...
class MorphologyContainerization(MorphologyContainerizationsForm, SingleCoordinateMixin):
    ...
    @staticmethod
    def _check_morph_folders(nodes, morph_folders_to_delete):
        ...
        for _morph_ext in ["h5", "asc", "swc"]:
            ...
            if (
                ...
                and len(self._filter_ext(Path(morph_folder).iterdir(), _morph_ext)) == 0
            ):
    ...
    @staticmethod
    def _convert_to_h5(morph_folders, morph_names):
    ...
    def _merge_into_h5_container(self, h5_folder, morph_names):
    ...
    @staticmethod
    def _set_global_morph_entry(global_morph_entry, cfg_dict):
        ...
        if "components" in cfg_dict:
            ...
            if global_morph_entry:
                ...
                h5_file = Path(base_path) / self.CONTAINER_FILENAME
                cfg_dict["components"]["alternate_morphologies"] = {"h5v1": h5_file}
        return global_morph_entry

    @staticmethod
    def _set_morph_entries_per_population(global_morph_entry, cfg_dict, nodes):
    ...
    def run(self, db_client: entitysdk.client.Client = None) -> None:  # noqa: ARG002
        ...
        for npop in node_populations:
            ...
            global_morph_entry = self._set_global_morph_entry(global_morph_entry, cfg_dict)

            # Otherwise, set individually per population
            self._set_morph_entries_per_population(global_morph_entry, cfg_dict, nodes)
