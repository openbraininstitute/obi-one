...
def _get_sonata_asset(db_client: Client, circuit_id: str):
...
def _fetch_file(db_client: Client, circuit_id: str, asset_id, rel_path: str, dest_dir: Path) -> Path:
...
def download_electrical_models(
    circuit_id: str, db_client: Client, dest_dir: Path, node_population: str | None = None
) -> list[Path]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        config_path = _fetch_file(db_client, circuit_id, asset.id, "circuit_config.json", Path(tmp))
        ...
        if node_population is not None:
            if node_population not in circuit.nodes.population_names:
                msg = (
                    f"Node population '{node_population}' not found in circuit '{circuit_id}'. "
    ...
    for pop, hoc_rel_path in pop_hoc_dirs:
        hoc_files = [
            path for path in file_list.files
            if path.startswith(hoc_rel_path + "/") and path.endswith(".hoc")
        ]

        if not hoc_files:
            if node_population is not None:
                msg = f"No .hoc files found in '{hoc_rel_path}' for population '{pop}' of circuit '{circuit_id}'."
                raise FileNotFoundError(msg)
            L.info(f"No .hoc files found for population '{pop}' - skipping.")
            continue

        # If downloading all populations, use subfolders
        if node_population is None:
            pop_dest_dir = dest_dir / pop
        else:
            pop_dest_dir = dest_dir

        for rel_path in hoc_files:
            downloaded.append(_fetch_file(db_client, circuit_id, asset.id, rel_path, pop_dest_dir))
...
def download_mechanisms(circuit_id: str, db_client: Client, dest_dir: Path) -> list[Path]:
    ...
    mod_files = [
        path for path in file_list.files
        if path.startswith(mechanisms_rel_path + "/") and path.endswith(".mod")
    ]
    ...
    for rel_path in mod_files:
        downloaded.append(_fetch_file(db_client, circuit_id, asset.id, rel_path, dest_dir))
...
def download_circuit_config(circuit_id: str, db_client: Client, dest_dir: Path) -> Path:
    ...
    """
    _, asset = _get_sonata_asset(db_client, circuit_id)
    return _fetch_file(db_client, circuit_id, asset.id, "circuit_config.json", dest_dir)
...
def download_node_sets(circuit_id: str, db_client: Client, dest_dir: Path) -> Path:
    ...
    return _fetch_file(db_client, circuit_id, asset.id, node_sets_rel_path, dest_dir)
...
def download_id_mapping(circuit_id: str, db_client: Client, dest_dir: Path) -> Path:
    ...
    Raises:
        FileNotFoundError: If no ID mapping file can be found.
    """
    _, asset = _get_sonata_asset(db_client, circuit_id)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        id_mapping_path = (
            config.get("components", {}).get("provenance", {}).get("id_mapping")
        )
    ...
    return _fetch_file(db_client, circuit_id, asset.id, rel_path, dest_dir)
