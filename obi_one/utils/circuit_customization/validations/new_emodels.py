...
def check_bluecellulab_initializable(
    paths: list[dict], circuit_id: str | uuid.UUID, proj_context, environment: str, access_token
) -> None:
    ...
    for path in paths:
        ...
        if not result_queue.empty():
            ...
            raise RuntimeError(
                "No result returned from subprocess when running bluecellulab instantiation check"
            )
...
def check_new_node_columns(old_node_file_path: str | Path, new_node_file_path: str | Path) -> None:
    ...
    assert new_attribute_names == old_attribute_names, (
        f"New node file has different attribute names than old node file. Old attribute names: {old_attribute_names}, New attribute names: {new_attribute_names}"
    )
    assert new_dynamic_attribute_names == old_dynamic_attribute_names, (
        f"New node file has different dynamic attribute names than old node file. Old dynamic attribute names: {old_dynamic_attribute_names}, New dynamic attribute names: {new_dynamic_attribute_names}"
    )

    old_selection = old_node_pop.select_all()
    new_selection = new_node_pop.select_all()
    assert (old_selection.flatten() == new_selection.flatten()).all(), (
        f"IDs have been modified in the new node file. Old IDs: {old_selection.flatten()}, New IDs: {new_selection.flatten()}"
    )
    ...
    for attr in old_attribute_names:
        if attr not in modifications_allowed_attrs:
            ...
            assert (old_values == new_values).all(), (
                f"Values of attribute {attr} have been modified in the new node file. Old values: {old_values}, New values: {new_values}"
            )
    ...
    for dyn_attr in old_dynamic_attribute_names:
        ...
        assert (old_values == new_values).all(), (
            f"Values of dynamic attribute {dyn_attr} have been modified in the new node file. Old values: {old_values}, New values: {new_values}"
        )
    ...
    for model_template in model_templates:
        ...
        if not old_fpath.exists() and not new_fpath.exists():
            raise FileNotFoundError(
                f"Hoc file for model template {model_template} not found in {old_hoc_dir} nor in {new_hoc_dir}"
            )
...
def check_new_hoc_in_nodes_file(
    ...
    for hoc_path in new_hoc_paths:
        ...
        assert hoc_path.suffix == ".hoc", (
            f"Expects hoc file to have .hoc extension, got {hoc_path.suffix}"
        )
        hoc_temp_name = f"hoc:{hoc_path.stem}"
        assert hoc_temp_name in model_templates, (
            f"Hoc template {hoc_temp_name} is not declared in the new node file. Templates declared in the new node file: {model_templates}"
        )
