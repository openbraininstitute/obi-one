...
- **`*.txt`** — generated by compiling the `*.in`. Do not edit by hand. The launch-system installs from these pinned files at task run time.

## Compiling

Regenerate the pinned `*.txt` files from their `*.in` sources with:

```bash
# Compile all launch-script requirements
make compile-launch-deps

# Compile a single file or directory only (FILE accepts an .in file or a directory)
make compile-launch-deps FILE=launch_scripts/launch_task_for_single_config_asset/dependencies/circuit_extraction.in
```

This runs `uv pip compile` resolving for the runtime platform (**linux/amd64, Python 3.12**), pinning the full transitive dependency closure. Some tasks depend on private packages (e.g. `ultraliser`, `neuromorphomesh`); compiling those requires AWS CodeArtifact authentication (the Makefile obtains a token automatically when AWS credentials are available).

Like `make compile-deps`, this **preserves the versions already pinned** in the committed `*.txt`, changing a pin only when the `*.in` (or obi-one's closure) forces it — so recompiling after an unrelated upstream release is a no-op. `entitysdk` is the exception: it is always upgraded to its latest version.

To deliberately bump the whole transitive closure to the latest versions (like `make upgrade-deps`), use:

```bash
# Upgrade all launch-script requirements to the latest versions
make upgrade-launch-deps

# Upgrade a single file or directory only
...
CI verifies that the committed `*.txt` files are consistent with their `*.in` sources (workflow `.github/workflows/check-launch-deps.yml`). Run the same check locally with:
